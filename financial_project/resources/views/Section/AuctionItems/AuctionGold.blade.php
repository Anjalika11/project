                ...
                <ul class="nav nav-underline-active-primary mb-3 justify-content-between" id="pills-tab12" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link active" id="pills-home-tab" data-toggle="pill" href="#nav-home-underline-primary"
                            role="tab" aria-controls="nav-tabs" aria-selected="true">Gold items reserved for auction</a>
                    </li>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-8 col-sm-6 ">
                                        <select class="form-control" aria-label="Default select example"
                                            ...
                                            <option value="Auctioned" style="background-color: rgb(253, 201, 201); ">
                                                Auctioned</option>
                                        </select>
                                    </div>
                                </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#ongoinLoan').DataTable({
            ...
            ajax: "{{ route('get-auction-gold-Loan') }}",
            ...
            rowCallback: function(row, data, index) {
                ...
                if (data.loan_status === 1) {
                    ...
                    if (rowEndDate < new Date(formattedToday)) {
                        $(row).find('td').css('background-color', 'rgb(255 172 172 / 49%)').css(
                            'color',
                            '#323232');
                    }
                }
        ...
        $('#completedLoan').DataTable({
            ...
            ajax: "{{ route('get-auction-gold-complete-Loan') }}",
            createdRow: function(row, data, dataIndex) {
                endDate = data.end_date;
                var today = new Date();
                today.setHours(0, 0, 0, 0);
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $.each(data, function(index, item) {
                        ...
                        var due_amount = parseInt(item.due_amount);
                        ...
                        var sum_pay_amount = parseInt(item.sum_pay_amount);
                        const interest = interest_rate;
                        var result = '';
                        var resultCount = 0;
                        if (interest_schema == 'For Each Months') {
                        ...
                        if (interest_schema == 'Daily') {
                            ...
                            var interest_rate_for_date = (interest_rate / 30) *
                                daysDiff;
                            var total_amount = parseInt(item.totalAmount);
